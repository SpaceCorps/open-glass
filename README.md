...
- **WebGL2 render pipeline in Rust** — `WebGl2Renderer` takes the `HtmlCanvasElement` and acquires the `webgl2` context itself through `web-sys`, compiles `kawase_blur.frag` and `glass_composite.frag` against a fullscreen-triangle vertex stage, and groups the submitted quads by `blurRadius` and issues, for each group, one dual-Kawase ping-pong blur per backdrop band (traversed to a depth derived from that group's `blur_radius`, up to 5 levels) followed by a rounded-rect composite of just that group's quads, with `OpticalParams` uniforms and alpha blending. Radii within 2px of each other share a group, groups are capped at `MAX_BLUR_GROUPS` (4) with the rarest radii snapped to the nearest kept one, and bands at `MAX_BACKDROP_BANDS` (3) — so a frame costs up to 12 blur chains where the old shared blur cost one, and a scene of panels that all agree on a radius still costs exactly one per band. **The composite refracts through both faces of the slab, everywhere on the panel.** It builds a front normal (the rim's SDF-gradient bevel plus the body's volumetric dome) and a rear normal (flat across the body, rolling into an inner bevel just inside the perimeter), then traces entry refraction, travel across `u_thickness` and exit refraction for each of R, G and B at its own index of refraction — so dispersion is now a real wavelength-dependent spread accumulated over two interfaces rather than one shared offset scaled per channel, and the rear face's inner bevel lights up the way the inside edge of thick glass does. Before this, `(1 - edge_proximity)` pinned every normal more than 8px inside the boundary to `(0, 0, 1)`, so the window body bent no light at all. The composite also applies a luma-preserving saturation term after the tint mix (`u_saturation`, default 1.8 — the `saturate(180%)` every CSS fallback literal uses) and then a multiplicative exposure gain (`u_brightness`, default 1.5) that buys back the luma the handover drops when the CSS overlay goes from alpha 0.22 to 0.05; the gain lands after the saturation clamp and before the additive sheen, so highlights are not scaled. The blur is calibrated so `blur_radius` R composites like CSS `blur(2R px)` (`packages/core/src/optical/physics.rs`'s `CSS_BLUR_PIXELS_PER_RADIUS` / `KAWASE_STEP_SCALE`), so the playground's 16px default reads like the `blur(32px)` literal it stands in for. Backdrop uploads from an `HTMLCanvasElement` or `OffscreenCanvas` are ingested by Rust. `createGlassEngine()` awaits `initWasmEngine()`, so the pipeline compiles its GLSL and draws in the browser.
- **Depth-banded backdrop.** The composite samples up to three backdrop rasters, ordered far to near, each with its own distance behind the glass (`u_band_depth_0/1/2`) and each blurred at the radius of the panel being drawn. `<GlassUnderlying depth={480}>` declares where a layer sits; every registered layer gets its own `DomCapturePipeline`, `assignBands` ranks them by depth, and a fourth layer merges into the nearest band rather than being dropped. The refraction offset is now `dual_surface_refraction_offset_at_depth`, so a band's parallax scales with its declared distance, and `BODY_LENS_DEPTH_SCALE` (1.2 x the panel's half-height) is demoted to the fallback for a band that declared nothing — which keeps a single-layer app on exactly the look it had before banding. Bands composite near-over-far by alpha, not averaged, so a card in band 1 occludes the wallpaper in band 0 where it covers it. `packages/core/tests/webgl2_browser.rs` measures the parallax directly: the same checkerboard uploaded at depth 40 and at depth 400 moves 238 of 260 interior samples, by a mean of 13.39 levels and a maximum of 65; forcing `resolveBackdropDepth` to ignore its argument — the pre-banding behaviour — drops that to 0 samples and 0.00. The cost is pass count: one blur chain per radius group per band, up to 12 per frame against the old single chain.
