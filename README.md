...
- **Playground app** — `apps/playground`, a Vite+ single-page app with a control for every optical parameter — including Frosting Saturation — and live-updating numeric readouts, a macOS-style desktop scene and a visionOS-style spatial scene.
...
- **The GPU composite is still less chromatic than the CSS fallback it replaces, though the gap has closed considerably.** Measured over the same 400x130 region of the "System Monitor" window body used by [Plan 00612](plan://00612)'s evidence harness, before and after adding the saturation term and recalibrating the blur ([Plan 00625](plan://00625)): per-channel σ went from 27.3 / 26.7 / 25.1 to 33.86 / 33.26 / 30.80, against the CSS capture's own 34.25 / 31.69 / 24.27 — matching or exceeding it on two of three channels and within 1.1% on the third. Mean per-pixel channel spread (`max(rgb) - min(rgb)`, averaged over the region) went from 11.0 to 19.02, against the CSS capture's 40.08 — closer, but still a real gap on this metric, which the σ improvement alone does not close. Edge energy also rose, from 2.73 to 6.19 against the CSS capture's 2.09: the dual-Kawase pyramid's bilinear up-magnification reintroduces high-frequency detail a true Gaussian blur would not, so the panel now reads sharper than a CSS blur at the same nominal radius even though its chroma and mid-scale structure are closer to matching. See the Roadmap for what's left.
...
6. **Closing the remaining chroma gap.** The GPU composite's per-channel σ now matches or exceeds the CSS fallback's, but its mean channel spread (19.02) still trails the CSS capture's (40.08) — see "Not implemented yet" above for the numbers. Candidates: a saturation default above 1.8 (departs from the `saturate(180%)` CSS parity this plan chose deliberately), or a wider mid-tone contrast curve alongside the saturation term rather than instead of a further blur retune.
